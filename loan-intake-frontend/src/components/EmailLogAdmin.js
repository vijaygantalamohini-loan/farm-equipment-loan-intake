...
function EmailLogAdmin() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const result = await emailAPI.getEmailLogs({ page, pageSize });
        if (!cancelled) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Email Logs</h2>
          <p className="text-sm text-gray-500">Queued, sent, and failed email activity.</p>
        </div>
      </div>
            ...
            {loading ? (
              ...
              logs.map(log => (
                <tr key={log.id}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{log.to}</td>
